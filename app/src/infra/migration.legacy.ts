/**
 * Reprise des données de l'ancienne application (v1) vers le nouveau schéma.
 *
 * ─────────────────────────────────────────────────────────────────────────
 * POURQUOI CE MODULE EST À PART
 * ─────────────────────────────────────────────────────────────────────────
 *
 * Ce code ne s'exécute qu'une fois dans la vie d'un utilisateur, et jamais
 * pour qui n'a pas connu l'ancienne version. Il pesait pourtant onze
 * kilo-octets du paquet d'entrée, téléchargés par tout le monde à chaque
 * ouverture. Il est donc chargé à la demande, par `initialiser`, et seulement
 * quand `migrer()` a constaté que de l'ancien est présent.
 *
 * ─────────────────────────────────────────────────────────────────────────
 ...
 *     qui serait migré, combien, et ce qui pose problème.
 *  2. INSTANTANÉ AVANT ÉCRITURE. `reprendreLegacy()` appelle
 *     `archiverAvantReprise()` — qui vit dans le noyau — AVANT la moindre
 *     conversion.
 *  3. IDEMPOTENCE. Le noyau ne rend `reprise-requise` que si le stockage de
 *     la nouvelle version est absent : relancer ne duplique ni n'écrase rien.
 *
 * Ce module ne SUPPRIME jamais les anciennes clés.
 */
...
/**
 * Reprend les données de l'ancienne application, instantané compris.
 *
 * Appelée seulement après que `migrer()` a rendu `reprise-requise`, donc
 * seulement quand le stockage de la nouvelle version est absent.
 */
export function reprendreLegacy(stockage: Stockage): ResultatMigration {
  const rapport = analyser(stockage);
  const bloquantes = rapport.anomalies.filter((a) => a.gravite === 'bloquante');
  if (bloquantes.length > 0) {
    return { statut: 'echec', motif: bloquantes.map((a) => a.message).join(' ') };
  }
  ...
  // L'instantané est écrit AVANT les faits migrés, et cet ordre n'est pas
  // négociable.
  const echecArchive = archiverAvantReprise(stockage);
  if (echecArchive !== null) return { statut: 'echec', motif: echecArchive };
